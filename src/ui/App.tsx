...
export function App() {
  const [mainTab, setMainTab] = useState<"input" | "outputs" | "history">("input");
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      if (input.trim()) void runRepair("manual");
    }, 900);
  ...
  async function loadPendingSelection() {
    ...
    if (typeof data.pendingSelectionText === "string") {
      ...
      await runRepair("selection", data.pendingSelectionText);
      setMainTab("outputs");
    }
  }

  async function runRepair(nextSource: InputSource = source, nextInput = input) {
    setBusy(true);
    try {
  ...
  async function pasteFromClipboard() {
    ...
    setInput(text);
    await runRepair("clipboard", text);
    setMainTab("outputs");
  }
  ...
  return (
    <main className={isFullPage ? "app appFull" : "app"}>
      ...
      <section className="mainTabs">
        <button className={mainTab === "input" ? "active" : ""} onClick={() => setMainTab("input")}>Input</button>
        <button className={mainTab === "outputs" ? "active" : ""} onClick={() => setMainTab("outputs")}>Outputs</button>
      ...
      {mainTab === "input" && (
        <section className="panel">
          <CodeEditor value={input} onChange={setInput} minHeight={isFullPage ? 520 : 340} />
          <div className="statusBar">
            <span>Source: {source}</span>
            <span>{busy ? "Processing..." : repairResult ? `Status: ${repairResult.report.status}` : "Idle"}</span>
            <button onClick={() => runRepair(source)}>
              <Play size={16} /> Run
            </button>
          </div>
      ...
      {mainTab === "outputs" && (
        <section className="panel">
          ...
          {repairResult?.ok ? (
            <CodeEditor value={output} readonly language={activeOutput.language} minHeight={isFullPage ? 620 : 440} />
          ) : (
            ...
            <article key={item.id} className="historyItem">
              <button
                onClick={() => {
                  ...
                  setSource("history");
                  void runRepair("history", item.rawInput);
                  setMainTab("outputs");
                }}
              >
